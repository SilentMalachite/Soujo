...
| review | `/soujo:review` | `$review` | 指定した範囲（なければ直近の層。未コミットの変更を含む）の指摘を絞らず全部1つの表で。原則の違反を先に |

| ファイル | 中身 | 上限 |
|---|---|---|
| `SPEC.md` | 原則（`- P1 <名前> — <1文>`。ほかの節・完了条件・既存のコードより優先する）・目的・非目標・受け入れ基準（`- A1 <1文>`）・技術判断 | 約100行。原則は7行 |
| `PLAN.md` | `- [ ] <層> — <完了条件>` | 1層1行 |
