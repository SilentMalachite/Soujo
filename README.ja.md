...
AI とのコーディングは途中で途切れる。使用上限、圧縮された文脈、会議、一日の終わり、もう一方のホストへの移動、何週間も別のことをしている間。計画と決定が会話の中にしかないと、次のセッションはそれを組み立て直すところから始まる。Soujo はそれを `.soujo/` の短い4ファイルに置き、層を終えるたびにコミットする。だから次のセッションは、どちらのホストでも、ファイルだけで続きから始められる。
