...
- `soujo map code` は、`.tsx`・`.jsx`・`.js`・`.mjs`・`.cjs` で JSX を要素として読む。ほかの拡張子では `<x>` は型になる。本文と引用符付きの属性値はコードではないので、`<p>from './a.js'</p>` を線にせず、本文のアポストロフィ（`<p>don't</p>`）が後ろを飲み込む文字列を始めることもない。要素の中の `{ }` の中身は今までどおりコードとして読み、2つの `{ }` は別々に読むので `<p>{require}{'./a.js'}</p>` も線にしない。タグは型引数（`<Box<T>>`）・コメント・属性の値に直接書いた要素（`icon=<Icon/>`）の先も続けて読む。値と型の位置を区別しないので、値が始まれる位置に書いた generic な関数型（`type X = <T>(a: T) => T`）は要素として読む。これを閉じるものが無いため、その後の `{ }` の外の import は全部消える。タグにならない形（`<T,>(x) => x`）は `<` の次からコードとして読み直す。読み直すのは `<` ごとに1回だけで、読み直す量が本文の長さを超えるファイルは、残りを要素なしで最後まで読む。
