...
- `soujo resume` は、見出し・空行・HTML コメントだけの `SPEC.md` を、どの版のテンプレートから複製したものでも未作成とみなし、`/soujo:spec` を案内する。これまでは今のテンプレートと同じ SPEC だけを未作成とみなしていたので、このテンプレートより前に作ったプロジェクトは `/soujo:plan` へ案内されるところだった。
