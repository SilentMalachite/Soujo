...
- `soujo map code` は、`<x>` が型になる `.ts`・`.mts`・`.cts` 以外で JSX を要素として読む。本文と引用符付きの属性値はコードではないので、`<p>from './a.js'</p>` を線にせず、本文のアポストロフィ（`<p>don't</p>`）が後ろを飲み込む文字列を始めることもない。要素の中の `{ }` の中身は今までどおりコードとして読む。値と型の位置を区別しないので、値が始まれる位置に書いた generic な関数型（`type X = <T>(a: T) => T`）は要素として読む。これを閉じるものが無いため、その後の `{ }` の外の import は全部消える。タグにならない形（`<T,>(x) => x`）は `<` の次からコードとして読み直す。
