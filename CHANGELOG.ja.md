...
- `resume` と `close` が案内するコマンドの中の層名を、スキルと同じく単一引用符で囲んだ1引数にし、`-` で始まる層名の前には `--` を置く。貼り付けて実行すると、中の `$(…)` ではなくその名前の層が対象になり、空白や引用符を含む名前も1つの引数になる。
- シェルが留まったまま現在のディレクトリが消されたとき、`soujo next check` と `soujo next show --hook` は何も出さず終了コード0で終わる（Stop フックがそこで失敗しない）。ほかのコマンドは `uv_cwd` ではなく1行でその旨を伝える。
- 書く前に閉じられたストリーム（`soujo resume | head -1`）では、`EPIPE` のスタックトレースと終了コード1ではなく、静かに終わる。
- エラーの中のホームディレクトリ以下のパスを `~` で示す。端末から貼り付けた行にユーザー名が残らない。
