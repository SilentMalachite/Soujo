...
- `soujo layer done` は `PLAN.md` の項目に完了条件がない層を拒否する。これまでは締めてしまい、次の `NEXT.md` の `確認:` を書く元がなかった。
...
- C1 領域の文字（U+0080〜U+009F。Latin-1 として読む端末がエスケープ列と解釈する）を、C0 制御文字と U+2028/2029 と同じく、出力と `LOG.md` の行で空白にする。
