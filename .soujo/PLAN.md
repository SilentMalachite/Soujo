...
- [ ] L2 state — state.ts の公開9関数(parseNext〜formatDate)が揃い、各関数に通るテストが1本以上ある
- [ ] L3 io — files.ts(.soujo/ 上方探索・読み書き)と git.ts(toplevel/status/add/commit/log -1)が一時ディレクトリのテストで通る
- [ ] L4 init-plan-log — templates/ が揃い、init(git トップレベル・上書きしない)・plan list/next・log add がテストで通る
...
- [ ] L9 skills — skills/ 7本(frontmatter は name/description のみ・4節×各3行以内)と agents/reviewer.md が揃う
- [ ] L10 hosts — 両マニフェスト・両マーケットプレイス・hooks.json が揃い、claude plugin validate . と validate_plugin.py が通る
