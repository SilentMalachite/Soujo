...
## L32〜L33 の Codex レビュー（6件・`21d8edf` の後の fix コミット `005d4f7`・全部直した）
| # | 指摘 | 直し方 |
|---|---|---|
| 1 | 原則の形が SPEC より緩く、`1. P1 名前 — 文`・`- P1 名前 -- 文` が通る（P2） | 原則は字下げなしの `-` と `—` だけを認める。PLAN の区切りの許容は流用しない。受け入れ基準は従来どおりどのリスト記号でもよい |
| 2 | `<!-- a --> 本文 <!-- b -->` をコメント行として丸ごと飛ばし、形の違いと7行超過を見落とす（P2） | `isCommentLine`：コメントを位置で順に読み、コメントの外に空白以外があれば節の行にする |
| 3 | 字下げした原則の重複キーを見落とし、字下げを直すまで出ない（P3） | 原則は字下げ・別記号の行からもキーを読み、形の違いと重複を同時に出す。受け入れ基準の字下げ項目は対象外のまま |
| 4 | 全層完了で NEXT.md を読めないときの resume が未テスト（P3） | NEXT.md をディレクトリにしたケースを追加 |
| 5 | `次: converge` で止まった close と、`converge` という層名の close の拒否が未テスト（P3） | 既存ケースを plan / converge で回し、拒否の表に converge を追加 |
| 6 | validateSpec の長大入力テストがない（P3） | 長いキー・空白列・多数のコメント・閉じないコメントを線形時間テストに追加 |
- Codex は読み取り専用で型検査・state テスト55件・I/O モック20件を実行し、統合テストと build は未実行と明記。1〜3 は修正前の `src/state.ts` で新テスト2件が落ちることを確認。全403件（402通過・1件スキップ）は手元で通過済み。

## L34 の Codex レビュー（8件＋後続層4件・`7e3af27` の後の fix コミット `287d057`）
| # | 指摘 | 直し方 |
|---|---|---|
| 1 | `# SPEC<!--\n-->本文` でコメント削除が本文を見出しに連結し、未作成とみなす（P2） | `specUnwritten` を行単位にし、コメントの閉じた行の後ろに文があれば本文とする |
| 2 | 4スペース字下げの `<!-- -->`（コード）やインラインコードの `<!--` も削除する（P2） | コメントは字下げ3つまでの行頭でだけ始める（CommonMark の HTML ブロック）。タブ字下げもコード |
| 3 | BOM 付きのテンプレートを作成済みとみなす（P2） | 先頭の BOM を読み飛ばす |
| 4 | `# SPEC <!-- 閉じない\n## 目的` が未作成になり、説明文と食い違う（P3） | 判定は変えない（CommonMark では見出しの行はそれで終わり、コメントは続かない）。説明文を「見出しは中身によらず見出し、閉じないコメントは行頭で始まるものが対象」に直した |
| 5 | setext 見出しだけの SPEC を作成済みとみなす（P3） | 判定は変えない（どの版のテンプレートも `#` 見出し）。SPEC(+ja) §6・§14 と CHANGELOG(+ja) に「`#` の見出し」と明記 |
| 6 | CR だけの改行で本文を見出しの行に含めてしまう（P3） | `\r\n`・`\r`・`\n` で行を分ける |
| 7 | 1〜6 の回帰テストと、閉じたコメントが多数ある長文のテストがない（P3） | 各入力を state テストに、長文を線形時間テストに追加 |
| 8 | 読めない SPEC の resume と、新旧テンプレートの brief が未テスト（P3） | resume（`/soujo:plan` のまま）と brief（`SPEC.md が未作成`）に追加 |
- 後続層：spec スキルと README の節一覧（L36 の完了条件にある）、go・skills テスト・README のスキル表（L35 の完了条件にある）。条件にない plan スキルの予約名・`.codex-plugin/plugin.json` の紹介文・CLAUDE.md(+テンプレート) の effort 行は L35 の NEXT の注意に入れた。SPEC(+ja) §5 の LOG 行で節目を書くスキルに converge が抜けていたのは、この fix で直した。
- Codex は読み取り専用で型検査・読み取りテスト57件・I/O 差し替え12件を実行し、`npm test` 全体は未実行と明記。1〜3・6 は修正前の `src/state.ts` で新テストが落ちることを確認。全404件（403通過・1件スキップ）は手元で通過済み。

## L35 の Codex レビュー（11件＋L36 予定4件・`a18eca2` の後の fix コミット `ea8c32a`）
| # | 指摘 | 直し方 |
|---|---|---|
| 1 | converge が NEXT.md を直す条件を警告の文面に頼り、NEXT.md がない・effort が無効・`次: spec` では追加した層へ進めない（P2） | NEXT.md がない・無効か、`次:` が最初の未完了層でないか、警告が `確認:` を指したら next set（plan も同じ。plan は未完了の層があるときだけ） |
| 2 | 未完了の層が残っていても収束にでき、`next set plan` の直後に警告が出る（P2） | 収束は「差も未完了の層もない」ときだけ。未完了の層だけ残る場合は、差が残る側と同じく NEXT.md をその層へ |
| 3 | `unrequested` だけ残るときの分岐が曖昧（P2） | 差を `missing`・`partial`・`contradicts` に限り、`unrequested` だけなら収束側 |
| 4 | キーのない SPEC で `（<キー> <種類>）` に入れるものがない（P2） | SPEC での呼び名（節と番号）をキーの代わりにし、節目の未決に「SPEC にキーがない」と書く |
| 5 | 既存の層が扱う差をキーの一致だけで判断する（P2） | 未完了の層の完了条件で解消するかで判断し、キーの一致は手がかりにする |
| 6 | `wip:` コミットの実装と、日本語の基準に英語の識別子で書いたコードを読まない（P2） | `layer:`・`wip:` コミットが変えたファイル、識別子にした語でも検索。見つからなければ `missing` |
| 7 | plan に追記だけの指示がなく、converge が足した未完了の層を書き換えうる（P2） | 既存の行は完了・未完了とも書き換えず、足すのは末尾だけ |
| 8 | PLAN が無効なとき converge に逃げ道がない（P3） | 最初に `soujo next check`。`PLAN.md の`・`PLAN.md を` で始まる警告があれば何も足さずに伝えて止める |
| 9 | converge の条件を消しても skills テストが通る（P2） | 本文の各条件を字句で検査し、converge の終わり方ごとの CLI の答え（警告・next set・resume）をテストに追加 |
| 10 | README が「差をすべて層にする」と読める（P3） | 未完了の層が解消しない差を最大12まで、残りは節目の未決と書く |
| 11 | README の未コミットの説明に converge がない（P3） | spec・plan・converge はコミットしない、差のない converge の後は SPEC が変わって層が締まるまで続くと追記 |
- L36 予定（完了条件にある）：spec の原則の1問とキー付け、go の原則との衝突時の1問と CLAUDE.md・AGENTS.md の原則優先、review と reviewer の原則違反を先に、README の SPEC 行の原則。
- SPEC(+ja) §7 の plan・converge 行を上に合わせた。Codex は読み取り専用で型検査と読み取りテスト35件を実行し、実ホストの確認は L37 と明記。1〜3・5〜9 の本文の字句は修正前の SKILL.md で新テストが落ちることを確認。全410件（409通過・1件スキップ）は手元で通過済み。

## L36 の Codex レビュー（9件・`aef6c60` の後の fix コミット `6eb3dae`）
| # | 指摘 | 直し方 |
|---|---|---|
| 1 | 既存の原則にキーだけ足すと `validateSpec` が形違いと警告する（P2） | 意味を変えずにこの形へ直し、原則には名前と `—` も補う。旧形式の原則が警告され、名前付きなら通ることをテストに追加 |
| 2 | 原則が決まらないとき、節目の未決の行から消える（P2） | 未決の行に「決まらず空のままの原則」も書く |
| 3 | キーのない SPEC で、review・reviewer が `P<n>` で始められない（P2） | converge と同じく、節と番号をキーの代わりにする |
| 4 | 「先頭の行にまとめ」が複数の違反を1行に集約する指示に読める（P3） | 1件1行で表の先頭から並べる |
| 5 | CHANGELOG(+ja) が原則の節を無条件に空にすると読める（P3） | 原則が決まらないときだけと明記。LOG.md の L36 エントリは追記のみの記録なので直さない |
| 6 | 空にする条件を消しても skills テストが通る（P3） | 条件付きの文全体を検査 |
| 7 | review が reviewer の表を加工しない指示が未検査（P3） | 「返った表を加工せずに出す」を検査 |
| 8 | CLAUDE/AGENTS の質問の条件を消しても hosts テストが通る（P3） | ホストごとの文面で条件ごと検査 |
