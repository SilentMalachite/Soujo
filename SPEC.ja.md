...
- `converge` は原則と基準を、利用者が実際に動かす経路（コマンド・起動のしかた）で判定し、その経路にない環境変数や設定だけで抑えた振る舞いを `met` の根拠にしない（L37）：収束した Codex の複製でもう一度実行すると、コピー設置から `agent-review --version` を起動すると `.pyc` が書かれることを見つけた。前回はテストが書き込みを抑えていたため、その原則（ファイルを書かない）を met としていた。収束の後の `converge` が層を足すことはなお起こる：判定はモデルの読みで、遅れて見つかった差も差であり、次の `go` がほかの差と同じく閉じる。
...
- `go` は完了条件が求めるものを変え、既存のテストがその変更に結びつけるファイルも含む：L3 は既存のテストが求めたため、導入先 SPEC の範囲外の `CHANGELOG.ja.md` も変え、そう報告した。完了条件を満たしたのであって広げたのではなく、SPEC にそのファイルを書くかは報告を見た利用者が決める。
- `soujo next set` は、`LOG.md` の最後の PLAN の層のエントリ（`layer done` と `close` が書くもの）より後に `節目` エントリがなければ、`次: plan` と `次: converge` を拒否する。そのエントリがなければどの節目でもよく、ほかの名前のエントリは数えない（L37）。スキルはもともと節目を先に残すが、Codex は、met のキーをスキルが定める2行を超えて足して節目を拒否された後、それを再実行する前に `next set --layer plan` を実行したことがあり、その間で止まっていれば、`brief` が示す節目なしにそのフェーズから再開していた。そこでモデルを信じず CLI が順を守らせ、`converge` は `--line` を2つにしてキーを1行目の括弧の中に書く。代わりに、最後の層を締めた後に `NEXT.md` を失って `次: converge` を書き直すには新しい節目が要る：`go` が残した節目はその層のエントリより前にあるため。
