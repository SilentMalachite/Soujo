...
| `soujo phase done <フェーズ>` | 4つの記録（`.soujo/` の SPEC・PLAN・LOG・NEXT と、その symlink の先、そこへ至る途中の symlink）だけを `phase: <フェーズ>` でコミットする。`spec` では、`soujo init` が置いたままで git がまだ追跡していないプロジェクト直下の `CLAUDE.md` / `AGENTS.md` も加える。`spec` / `plan` / `converge` が `next set` で引き継いだ後に使う（`spec` は `次:` が未完了の層なら next set なしでも）。プロジェクトのほかの変更は、ステージ済みかどうかによらずそのまま残す（フェーズの名前でほかの作業をコミットしない）。次のときは何もコミットせずに終了1：フェーズが `spec` / `plan` / `converge` でない／NEXT.md がない・無効・`次:` がまだこのフェーズ（コミットに次の一手が入るように）／layer done のコミット不能条件（認証情報のファイル名はコミットするものの中だけで検査する：未追跡の記録の symlink の先がその名前なら拒否）／PLAN のチェックが未コミット（途中で止まった layer done。`close` と同じく先にそれを再実行）／`LOG.md` の書庫に未コミットの変更がある（途中で止まった log rotate。先にそれを再実行）／記録が書いたとおりにステージされていない（skip-worktree など）。最後の検査とコミットを止めた git の hook はステージの後なので、ステージしたものはステージ済みのまま残る。コミットするものに未コミットの変更がなければそう言って終了0（成功の後の再実行は害がない）。拒んだ hook の原因を直して再実行すればコミットする | 1行 |
