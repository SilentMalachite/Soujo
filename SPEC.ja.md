...
| `converge` | `SPEC.md`（その `A<n>` / `P<n>` の行。なければ基準と原則を持つ節）、`PLAN.md`、プロジェクトのコードのうち層のコミットが変えたものと基準の語で検索して見つかるもの。それより先は読まない | 基準と原則を1つずつ、満たす・`missing`・`partial`・`contradicts` に分け、`path:line` の根拠を付ける。SPEC にも PLAN にもない実装は `unrequested`。SPEC もコードも変えない。未完了の層が受け持たない差を、既存の行を変えずに PLAN へ層として足し、完了条件の末尾にキーと種類を付ける（`（A3 partial）`）。原則違反を先に置き、未完了は `plan` と同じく最大12層。`unrequested` と上限を超えた差は層にせず、節目の未決の行に出す | 差が残れば（層を足したときも、未完了の層がすでに受け持つときも）：`soujo log add 節目`（どのキーか・何層か・何が未決か）→ `soujo next check` → `soujo next set`（その警告が `次:` か `確認:` を指したときだけ）→ `soujo map plan`。差がなければ：`soujo log add 節目`（収束：何を照らしたか・何が未決か）→ `soujo next set`（次: plan） | 全部の差の表 `キー / 種類 / 根拠 / 残り`、続けて図か収束の1行 | high |
