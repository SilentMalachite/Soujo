...
| `soujo phase done <フェーズ>` | 4つの記録（`.soujo/` の SPEC・PLAN・LOG・NEXT とその symlink の先）だけを `phase: <フェーズ>` でコミットする。`spec` / `plan` / `converge` が `next set` の後に使う。プロジェクトのほかの変更は、ステージ済みかどうかによらずそのまま残す（フェーズの名前でほかの作業をコミットしない）。次のときは何もコミットせずに終了1：フェーズが `spec` / `plan` / `converge` でない／NEXT.md がない・無効・`次:` がまだこのフェーズ（フェーズは先に `next set` で引き継ぐので、コミットに次の一手が入る）／認証情報のファイル名を除く layer done のコミット不能条件（それはプロジェクト全体をステージするときだけ取り込むもの）／記録が書いたとおりにステージされていない（skip-worktree など）。4つの記録に未コミットの変更がなければそう言って終了0（成功の後の再実行は害がない）。コミットを止めた git の hook は1行で示し、原因を直して再実行すればコミットする | 1行 |
...
|---|---|---|---|---|---|
| `spec` | `soujo init` 後の `SPEC.md`。技術の候補を出すときは設定ファイル。原則の候補を出すときは設定ファイルとリポジトリが明文化している規約 | 質問1つずつ（答えを受けてから次）・最大7問。各問に番号付きの答え候補。答えるたびに、次の問いの前に書く。技術スタックは設定ファイルで決まればそれ、決まらなければ質問（既定値を持たない）。原則は、それまでの答え・設定ファイル・明文化された規約から出した候補で1問にして決める。受け入れ基準はキー付きで書き、既存の SPEC で欠けたキー（原則には名前も）は意味を変えずに付ける。原則が決まらなければ、その節は `未定` と書かず空のままにする | `soujo init` → `soujo log add 節目`（SPEC を書いた・原則を変えたらそれも・決まらない原則を含めて何が未決か）→ `soujo next set`（次: plan）→ `soujo phase done spec` | `SPEC.md` の骨組みの表1つと init が作ったファイル | high |
| `plan` | `SPEC.md` と既存の `PLAN.md`（その行は書き換えない） | 未実装を30分以内の層に分割。依存順。未完了は最大12層。未実装がなければ層を足さず終える | `soujo log add 節目`（層を足したときだけ：何層か・何を外したか）→ `soujo next check` → `soujo next set`（未完了の層があって、`NEXT.md` がない・無効か、`次:` が最初の未完了層でないか、その警告が `確認:` を指したときだけ）→ `soujo phase done plan`（層を足したときだけ）→ `soujo map plan` | `PLAN.md` と図 | high |
...
- `spec` / `plan` / `converge` は `next set` の後に `soujo phase done '<フェーズ>'` で記録を `phase: <フェーズ>` としてコミットする。次の `layer done` まで残すと、差のなかった `converge` の後は SPEC が変わって層が締まるまで未コミットが続いたため。コミットするのは4つの記録だけ：フェーズはコードを変えず、ほかの場所の変更は別の誰かの作業で、フェーズの名前でコミットすると取り違えるから。そのため未追跡の認証情報名は検査しない。`log add` や `next set` のついでではなく独立したコマンドにして、各コマンドの役割を1つに保つ。`next check` は `次:` がフェーズの間、`phase done` より前にフェーズが書いた分のために件数から `.soujo/` を外したまま。それ以外の場所の変更は今までどおり警告し、数えるのはそれだけ。`phase:` コミットはどの層のものでもない：`review` と `map diff` は層のコミットとみなさず、`brief` は最後の層の後のコミットに数える。
