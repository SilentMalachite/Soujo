...
- git の呼び出しはどれも120秒で打ち切り、ほかの git の失敗と同じ1行で伝える。`commit.gpgSign` の pinentry が読む先を持たないときや、戻らない hook があるときに、`soujo` を呼んだセッションを止めたままにしないため（CLI 側が終わるだけでは終わらない）。`next check --hook` のときだけは git 呼び出し全体で20秒にする。`hooks/hooks.json` の Stop に与えられた時間の内側に収めることで、巨大な作業ツリーで `git status` が間に合わないときも、警告ごとホストに殺されるのではなく警告の1件として出す。
