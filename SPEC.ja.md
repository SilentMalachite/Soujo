...
| 性質 | 要件 | 満たす仕組み |
|---|---|---|
| Interruptible development | どこで止めても、作業も決定も失わない | 1層=1コミット。`soujo close` が途中の作業を `wip:` でコミット。`soujo phase done` がフェーズの記録を `phase:` でコミット。`soujo next check` が再開できない状態を警告（§6） |
...
| `LOG.md` | 日誌。`## YYYY-MM-DD 層名`（実在する日付）の下に最大3行。コマンドは追記だけで、例外は過去の月を書庫へ移す `soujo log rotate`。層名が `節目` のエントリは節目：どのフェーズが終わり何が未決かを、`spec` / `plan` / `go` / `converge` スキルがフェーズの区切りで書き、`soujo brief` が示し、`soujo next set` が `plan` / `converge` の前に求める（§6） | 1エントリ3行 | `soujo log add` / `layer done` / `close --note`。`soujo log rotate` が削る |
...
- git は、別のリポジトリを指す環境変数（`git rev-parse --local-env-vars` が挙げるもの：`GIT_DIR`・`GIT_WORK_TREE`・`GIT_INDEX_FILE` など）と、pathspec の読み方を変える環境変数（`GIT_LITERAL_PATHSPECS`・`GIT_GLOB_PATHSPECS`・`GIT_NOGLOB_PATHSPECS`・`GIT_ICASE_PATHSPECS`）を外して実行する（環境変数名の大小文字を区別しない Windows では、設定された綴りのまま外す）ので、git のフックから起動した `soujo` もカレントディレクトリのリポジトリを扱い、自分の pathspec は書いたとおりに読まれる。git 管理外とみなすのは git の「not a git repository」のときだけ。それ以外の git の失敗（git がない・設定が壊れている）はエラーで、`resume` と `brief` は git の行に、`next check` はほかの警告と並べて1件の警告に出し、`log add` は git 管理外と同じく HEAD 不明として扱う。
...
| `soujo next set --layer --premise --check [--caution] [--effort]` | `NEXT.md` を全文書き直す。既定は 注意=`なし`、effort=`medium`。フェーズ `spec` / `plan` / `converge`（前後の空白を除き、大文字小文字も含めて完全一致で比べる）の effort は `high`（§7）：`high` 以外の `--effort` は拒否して何も書かず、エラーは `--effort` を外すよう示す。値は制御文字（§14）を含まない1行。PLAN に層があるとき、PLAN にない層（フェーズを除く）は何も書かずに拒否。PLAN にコードフェンスの閉じ忘れ・空の層名・制御文字を含む層名がある（3つとも行番号で示す）、同じ層名が複数ある、または `spec` / `plan` / `converge` / `節目` という層がある間は、どの層も何も書かずに拒否。`plan` / `converge` は、`LOG.md` の PLAN の層名のエントリのうち最後のものより後に `節目` エントリがなければ（そのエントリがなければ節目が1つもなければ）何も書かずに拒否し、エラーは先に実行する `soujo log add '節目'` を示す | 1行 |
| `soujo next check [--hook]` | NEXT.md がない・無効・PLAN で `[x]` 済みの層か未チェックの層より後ろを指す（`plan` と `converge` はすべての層より後ろ）、PLAN にコードフェンスの閉じ忘れ・空の層名・制御文字を含む層名がある（3つとも行番号で示す）・同じ層名が複数ある・`spec` / `plan` / `converge` / `節目` という層がある、完了条件のない層がある（行番号で示す。ここでは警告、`layer done` では拒否）、その層自体が成り立っているのに `NEXT.md` の `確認:` が PLAN のその層の完了条件と違う（空白の連なりは制御文字も含めて同じと見る。フェーズ・PLAN にない層・PLAN が重複させている層・完了条件のない層、および完了済みか手前を飛ばした層——そちらの警告の方が直す先——は比べない）、`SPEC.md` の `## 原則` が7行を超えるか `- P<n> <名前> — <1文>` でない行を持つ・その `## 受け入れ基準` の字下げのない項目に `A<n>` のキーがない・キーが節の中で重複する（空行・1行の HTML コメント・コードフェンスはこれらの節の行に数えない。行数以外は行番号で示す。この見出しのない SPEC は検査しない）、プロジェクト内に未コミット変更がある（未追跡のディレクトリは `status.showUntrackedFiles` の設定によらず1件と数える。`git status` の出力が 16 MiB を超えたら `N件以上` と出す。`次:` が `spec` / `plan` / `converge` の間は `.soujo/` を数えない。これらのフェーズは `phase done` がコミットする前に記録を書くため）、`soujo log rotate` が2か月分以上のエントリを移せる、または `SPEC.md`・`NEXT.md`・`PLAN.md`・`LOG.md` のどれかを読めない（どれもほかの警告と並べて出し、そのファイルが要る検査だけを外す）ときに警告。行番号の警告は壊れた行の数だけ出るので、1行には先頭4件と `ほか<N>件` を出す。Soujo を使っていないプロジェクトでは無音。`--hook` 時は `{"systemMessage": "..."}`（ホームディレクトリを隠してから組む）。**終了コードは常に0** | 0〜1行 |
...
| `soujo close [--note ...]` | `--note` を LOG に「中断: ...」で記録 → プロジェクトを `wip: <層名>` でコミット → 再開方法。次のときは何も書かずに終了1：layer done と同じコミット不能条件／layer done が拒否する PLAN の問題（コードフェンスの閉じ忘れ・空の層名や重複した層名・制御文字を含む層名・`spec` / `plan` / `converge` / `節目` という層）／NEXT.md がない・無効・チェック済みの層を指す／PLAN のチェックが未コミット（先に layer done を再実行）／note が空・LOG の上限を破る。SPEC・PLAN・LOG・NEXT が書いたとおりにステージされていない（skip-worktree など。index に残ったまま消えたものも）間は何もコミットしない。層は `次:`、ただし `次:` が未チェックの層より後ろを指すとき（next set と layer done の間で止まった）はその未チェックの層。HEAD にまだないその層の「中断」エントリが LOG のどこかにあれば追記せず残すので、コミット失敗後の再実行はコミットだけやり直す | 2行 |
| `soujo phase done <フェーズ>` | 4つの記録（`.soujo/` の SPEC・PLAN・LOG・NEXT と、その symlink の先、そこへ至る途中の symlink）だけを `phase: <フェーズ>` でコミットする。`spec` では、プロジェクト直下の `CLAUDE.md` / `AGENTS.md` も、HEAD になく、`soujo init` が複製するテンプレートと1バイトも違わない間だけ加える（ステージ済みでもよく、git に無視されていないもの）。利用者自身のファイルや、その後に直したものは加えない。`spec` / `plan` / `converge` が `next set` で引き継いだ後に使う（`spec` は `次:` が未完了の層なら next set なしでも）。プロジェクトのほかの変更は、ステージ済みかどうかによらずそのまま残す（フェーズの名前でほかの作業をコミットしない）。次のときは何もコミットせずに終了1：フェーズが `spec` / `plan` / `converge` でない／NEXT.md がない・無効・`次:` がまだこのフェーズ（コミットに次の一手が入るように）／layer done のコミット不能条件（認証情報のファイル名はコミットするものの中だけで検査する：未追跡の記録の symlink の先がその名前なら拒否）／PLAN のチェックが未コミット（途中で止まった layer done。`close` と同じく先にそれを再実行）／`LOG.md` の書庫に未コミットの変更がある（skip-worktree や assume-unchanged で隠れたものも。途中で止まった log rotate。先にそれを再実行）／記録の symlink をたどれない（先がないかループ）／途中の symlink が、HEAD か index でディレクトリの場所にある（ステージするとそのファイルを消す。`layer done` か `close` で全体とコミットする）／記録が書いたとおりにステージされていない（skip-worktree など。index に残ったまま消えたものも）。最後の検査とコミットを止めた git の hook はステージの後なので、ステージしたものはステージ済みのまま残る。ステージの直前に、止まったプロセスが残した一時ファイルをほかのコミットするコマンドと同じく消すので、拒否した実行は何も消さない。コミットするものに未コミットの変更がなければそう言って終了0（成功の後の再実行は害がない）。拒んだ hook の原因を直して再実行すればコミットする | 1行 |
...
|---|---|---|---|---|---|
| `spec` | `soujo init` 後の `SPEC.md`。技術の候補を出すときは設定ファイル。原則の候補を出すときは設定ファイルとリポジトリが明文化している規約 | 質問1つずつ（答えを受けてから次）・最大7問。各問に番号付きの答え候補。答えるたびに、次の問いの前に書く。技術スタックは設定ファイルで決まればそれ、決まらなければ質問（既定値を持たない）。原則は、それまでの答え・設定ファイル・明文化された規約から出した候補で1問にして決める。受け入れ基準はキー付きで書き、既存の SPEC で欠けたキー（原則には名前も）は意味を変えずに付ける。原則が決まらなければ、その節は `未定` と書かず空のままにする | `soujo init` → `soujo log add 節目`（SPEC を書いた・原則を変えたらそれも・決まらない原則を含めて何が未決か）→ `soujo next set`（次: plan。`次:` が未完了の層ならしない）→ `soujo phase done spec`（next set なしでも。`git init` を断られたらしない） | `SPEC.md` の骨組みの表1つと init が作ったファイル | high |
| `plan` | `SPEC.md` と既存の `PLAN.md`（その行は書き換えない） | 未実装を30分以内の層に分割。依存順。未完了は最大12層。未実装がなければ層を足さず終える | `soujo log add 節目`（層を足したときだけ：何層か・何を外したか）→ `soujo next check` → `soujo next set`（未完了の層があって、`NEXT.md` がない・無効か、`次:` が最初の未完了層でないか、その警告が `確認:` を指したときだけ）→ `soujo phase done plan`（`次:` が plan でなくなったとき：層を足したか next set をした）→ `soujo map plan` | `PLAN.md` と図 | high |
| `go` | `soujo resume` → `NEXT.md` → `SPEC.md` → PLAN の該当層 | `再開:` が go 以外ならそれに従う（`soujo brief` の案内には従わない）。層を完了条件まで実装。原則に反さないと完了条件を満たせないときは、実装せず1問だけ聞く。**終わったら次の層の `NEXT.md` を先に書き（最後の層の後は `次: converge`）、その後 `layer done`。最後の層では `NEXT.md` の前に `節目` エントリを LOG へ書く**。拒否されたコマンドは失敗したところから再実行し、節目がないと拒否された `next set` の前には節目を残し直す。NEXT が spec / plan / converge を指すならそちらへ | `soujo resume` → `soujo log add 節目`（最後の層だけ：層で何ができたか・何が未決か）→ `soujo next set` → `soujo layer done` | 結果1文＋変更ファイル | `NEXT.md` の指定 |
| `converge` | `SPEC.md`（その `A<n>` / `P<n>` の行。なければ基準と原則を持つ節）、`PLAN.md`、プロジェクトのコードのうち `layer:`・`wip:` コミットが変えたものと基準の語（識別子にした語も）で検索して見つかるもの、利用者が動かす経路を決めるファイル（エントリポイント・パッケージの設定・README の起動方法）。それより先は読まない | 基準と原則を1つずつ、満たす・`missing`・`partial`・`contradicts` に分け、`path:line` の根拠を付ける（コードが見つからなければ `missing`）。判定は利用者が実際に動かす経路で行い、その経路にない環境変数や設定だけで抑えた振る舞いを満たすとする根拠にしない。SPEC にも PLAN にもない実装は `unrequested`（`soujo init` が置いたファイルと `.soujo/` は除く）。SPEC もコードも変えない。キーのない SPEC では SPEC での呼び名（節と番号）をキーの代わりにし、節目の未決の行にキーがないことを書く。差（`missing`・`partial`・`contradicts`）のうち未完了の層の完了条件では解消しないもの（キーの一致は手がかり）を、既存の行を完了・未完了とも変えずに PLAN へ層として足し、完了条件の末尾にキーと種類を付ける（`（A3 partial）`）。原則違反を先に置き、未完了は `plan` と同じく最大12層。`unrequested` と上限を超えた差は層にせず、節目の未決の行に出す | 最初に `soujo next check`：`PLAN.md` についての警告があれば何も足さず、その警告を示す。差か未完了の層が残れば：`soujo log add 節目`（どのキーか・何層足したか・何が未決か）→ `soujo next check` → `soujo next set`（`NEXT.md` がない・無効か、`次:` が最初の未完了層でないか、その警告が `確認:` を指したときだけ）→ `soujo phase done converge` → `soujo map plan`。どちらもなければ（`unrequested` だけでも）：`soujo log add 節目`（収束：何を照らしたか・何が未決か）→ `soujo next set`（次: plan）→ `soujo phase done converge` | 全部の差の表 `キー / 種類 / 根拠 / 残り`、続けて図・収束の1行・止めた `PLAN.md` の警告のどれか | high |
...
| コミット | 通常の権限で可 | `workspace-write` サンドボックスは `.git` に書けない。`layer done` / `close` / `phase done` は承認が要る（`codex exec` なら `--add-dir "$PWD/.git"`）。再実行でコミットだけやり直る |
...
- `soujo close` と `soujo phase done` は PLAN のチェックが未コミットなら拒否する。途中で止まった layer done が `wip:` や `phase:` ではなく自身の `layer:` コミットで終わるようにするため。
...
- `layer done`・`close`・`phase done` はコミット前に SPEC・PLAN・LOG・NEXT が書いたとおりにステージされたかを確かめる。それらを欠いたままコミットすると、`layer done` の再実行はコミット済みとして拒否され、`close` は「中断」エントリのない、または古い `NEXT.md` の `wip:` コミットを残し、スキルが直した `SPEC.md` は黙ってその層から漏れ、`phase:` コミットは自分の記録を欠くため。
...
- コミットはリポジトリの git hook を通す（`--no-verify` を付けない）。秘密情報の検査のようにコミット内容を守る hook は、層・`wip:`・`phase:` のコミットも守るべきだから。hook が拒めば記録を書いた後にコミットが失敗し、エラーがそう示すので、原因を直して再実行すればコミットされる。ほかのファイルをステージする hook は、`phase done` が記録だけをコミットする場合も含めて、そのファイルをコミットに入れる。git はパスを限ったコミットの hook を一時的な index で走らせるので、そのファイルについて index と HEAD がずれたまま残る。
...
- git の呼び出しはどれも120秒で打ち切り、ほかの git の失敗と同じ1行で伝える。`commit.gpgSign` の pinentry が読む先を持たないときや、戻らない hook があるときに、`soujo` を呼んだセッションを止めたままにしないため（CLI 側が終わるだけでは終わらない）。`next check --hook` のときだけは git 呼び出し全体で20秒にする（戻らない時計で測る）。`hooks/hooks.json` の Stop に与えられた時間の内側に収めることで、巨大な作業ツリーで `git status` が間に合わないときも、警告ごとホストに殺されるのではなく警告の1件として出す。予算を使い切った後の呼び出しは、起動せずに時間切れとして返す（もう1回分の git を待てば予算を越えるため）。
- 認証情報のファイル名は、どのファイルシステムでも小文字にして比べる（大小文字を無視する FS に限らない）。`.ENV` と `.env` が別物の FS でコミットしたリポジトリを、同じ物とみなす FS に clone すると、`.env` を読む道具が `.ENV` としてコミットされた中身を読むため。名前は道具が名前で読むもの（`.envrc`・`credentials`・`secrets.yml`・鍵束の `*.p12` / `*.pfx`）まで含め、中身は走査しない。コミットはリポジトリの git hook を走らせるので、秘密スキャナがあればこのコミットも守るため。
- bisect も途中の操作として扱う。HEAD を切り離すので、その最中のコミットは `git bisect reset` で置き去りになるため。
- 未追跡の認証情報名は `git add -A` の直前にもう一度検査する。`layer done`・`close`・`log rotate` は最初の検査とステージの間に記録を書くため。残る隙間は git の一覧とステージの間だけ。
...
- スキルはコマンドをツール呼び出し1回に1つずつ実行し、`&&`・`||`・`;` でつながない。`agents/reviewer.md` も同じ。Bash の許可リストのもとで、`spec` の最初のつないだコマンド（`command -v soujo && soujo init; ls; …`）が L12 と L37 で拒否され、モデルは単独のコマンドでやり直したため。
- `spec` は答えを得るたびに、次の問いを出す前に該当節へ書く。中断しても答えが残るように。最後にまとめて `SPEC.md` を書きがちだった（L12 では再現せず）。
- `review` は `soujo:reviewer` の表を、パスを縮めず・セルを言い換えず・語句を落とさず・前置きの文を足さずに出し、スキルはその4つを名指しする。L37 の Claude Code は指摘を順に全部残しつつ、4つともしたため。`soujo:reviewer` は場所に作業ツリーからの相対 `path:line` を書き、以前のように絶対パスは書かない。
- `converge` は原則と基準を、利用者が実際に動かす経路（コマンド・起動のしかた）で判定し、その経路にない環境変数や設定だけで抑えた振る舞いを `met` の根拠にしない（L37）：収束した Codex の複製でもう一度実行すると、コピー設置から `agent-review --version` を起動すると `.pyc` が書かれることを見つけた。前回はテストが書き込みを抑えていたため、その原則（ファイルを書かない）を met としていた。その経路を決めるファイル（エントリポイント・パッケージの設定・README の起動方法）は、層が変えていなくても読む。収束の後の `converge` が層を足すことはなお起こる：判定はモデルの読みで、遅れて見つかった差も差であり、次の `go` がほかの差と同じく閉じる。
- `converge` は `soujo init` が置いたファイル（`CLAUDE.md` / `AGENTS.md`）と `.soujo/` を `unrequested` に数えない（L37）：Claude Code は `soujo init` の `CLAUDE.md` をそこに挙げ、Codex は挙げなかった。スキルは `unrequested` をコードについて定めている。
- `converge` が読む文脈の量は、層の総和を SPEC に照らす唯一の工程の費用として受け入れる（L37：Codex では `$converge` 単独で入力48.8万トークン（キャッシュ44.3万）、`converge` を続けた `$go` で69.1万（キャッシュ64.1万）。Claude Code では `converge` を続けた `go` が35ターンでキャッシュから123万トークンを読んだ（ターンの合計））。読むコードは、スキルがすでに `layer:`・`wip:` コミットが変えたファイルと基準の語で見つかる箇所に限っている。
- Codex の `$go` 1回の文脈量（入力約30万〜69万トークン、大半キャッシュ）は主に Codex 全体の文脈から来るもので、Soujo の手は届かない。L12 では無関係なグローバルスキルも読み、Codex のメモリファイルを検索した。
- `go` は完了条件が求めるものを変え、既存のテストがその変更に結びつけるファイルも含む：導入先プロジェクトの L3 の `go` は、既存のテストが求めたため、導入先 SPEC の範囲外の `CHANGELOG.ja.md` も変え、そう報告した。完了条件を満たしたのであって広げたのではなく、SPEC にそのファイルを書くかは報告を見た利用者が決める。
- `soujo next set` は、`LOG.md` の PLAN の層名のエントリ（`layer done` と `close` が書くもの。その名前の `log add` も）のうち最後のものより後に `節目` エントリがなければ、`次: plan` と `次: converge` を拒否する。そのエントリがなければどの節目でもよく、ほかの名前のエントリは数えない（L37）。スキルはもともと節目を先に残すが、Codex は、met のキーをスキルが定める2行を超えて足して節目を拒否された後、それを再実行する前に `next set --layer plan` を実行したことがあり、その間で止まっていれば、`brief` が示す節目なしにそのフェーズから再開していた。そこでモデルを信じず CLI が順を守らせ、`converge` は `--line` を2つにしてキーを1行目の括弧の中に書く。守るのはこの2つのフェーズへの入口だけ：`plan` が足した最初の層への `next set` の前に残す節目と、層を足した `converge` が残す節目はスキルに任せる。`next check` は節目を探さない：順が崩れうるのは `next set` だけだから。また、最後の節目より後の過去の月の層のエントリを `log rotate` が移し、最後にほかの名前のエントリがあると、検査が見るエントリが減る。代わりに、後の層のエントリより前の節目は数えないので、`go` が残した節目の後に最後の層のエントリが入ってから（その層の `close`、または `NEXT.md` を失った後のその `layer done`）`次: converge` を書き直すには新しい節目が要る。エラーがそれを求め、`go` が残し直す。
- `spec` / `plan` / `converge` は `next set` の後に `soujo phase done '<フェーズ>'` で記録を `phase: <フェーズ>` としてコミットする。次の `layer done` まで残すと、差のなかった `converge` の後は SPEC が変わって層が締まるまで未コミットが続いたため。コミットするのは4つの記録と、それが HEAD で成り立つのに要るもの（symlink の先と途中の symlink）、`spec` では `soujo init` が置いた `CLAUDE.md` / `AGENTS.md` のうち HEAD になくテンプレートと1バイトも違わないもの（init が残した利用者のファイルや、その後に直したものは誰かの作業。残すと `plan` の間ずっと警告が続く）だけ：フェーズはコードを変えず、ほかの場所の変更は別の誰かの作業で、フェーズの名前でコミットすると取り違えるから。そのため認証情報のファイル名はコミットするものの中だけで検査する。ほかのコマンドが途中で残したものは、フェーズの名前でコミットせず拒否する：止まった `layer done`（`phase:` コミットがその PLAN のチェックを自身の `layer:` コミットから奪う）と、止まった `log rotate`（コミットした `LOG.md` に書庫が置いていかれる）。`log add` や `next set` のついでではなく独立したコマンドにして、各コマンドの役割を1つに保つ。`next check` は `次:` がフェーズの間、`phase done` より前にフェーズが書いた分のために件数から `.soujo/` を外したまま。それ以外の場所の変更は今までどおり警告し、数えるのはそれだけ。`phase:` コミットはどの層のものでもない：`review` と `map diff` は層のコミットとみなさず、`brief` は最後の層の後のコミットに数える。フェーズの `next set` と `phase done` の間で止まったセッションはコミットせずに進むので、`resume` は先に `phase done` を示す。先をたどれない記録の symlink と、HEAD でディレクトリだった場所の symlink は拒否する：コミットが HEAD に古い記録を残すか、そのディレクトリのファイルを消すため。
