...
- `soujo` refuses to run inside a host's plugin directory, where the real path of the current directory has `.claude/plugins` or `.codex/plugins`: every command but `--help` exits 1 with one line before reading or writing anything, and `soujo next show` / `soujo next check` behave as outside a Soujo project (nothing with `--hook`). Before, a model that moved into the installed copy of Soujo got this repository's records from `soujo resume`, and `soujo init` or a commit landed in the copy.
