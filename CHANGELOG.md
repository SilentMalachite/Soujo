...
- The `spec` skill settles the principles in one question, from candidates drawn from the answers, the config files, and the conventions the repository states, and writes `SPEC.md` in five sections with principles as `- P<n> <名前> — <判定できる1文>` (at most 7) and top-level acceptance criteria as `- A<n> <判定できる1文>`. Keys are never reused or renumbered; an existing SPEC gets missing keys without its meaning changing, and a missing heading is added. The principles section is left empty rather than `未定`, which `soujo next check` would warn about. Its `節目` entry says which principles changed.
...
- The `review` skill and the `soujo:reviewer` agent read the principles of `SPEC.md` and list their violations first, each starting with its key (`P<n>`), since review shows the reviewer's table unedited.
