...
- `soujo map code` reads a JSX element as an element in `.tsx`, `.jsx`, `.js`, `.mjs`, and `.cjs`; in every other extension `<x>` is a type. Its text and its quoted attribute values hold no code, so `<p>from './a.js'</p>` is no longer drawn as an edge and an apostrophe in the text (`<p>don't</p>`) no longer starts a string that swallows what follows. What a `{ }` inside an element holds is still read as code, and two of them are kept apart, so `<p>{require}{'./a.js'}</p>` is no edge either. A tag is read on through its type arguments (`<Box<T>>`), a comment, and an element written straight as an attribute's value (`icon=<Icon/>`). Value and type positions are not told apart, so a generic function type where a value can start (`type X = <T>(a: T) => T`) is read as an element; nothing closes it, and every import after it outside a `{ }` is lost. What cannot be in a tag (`<T,>(x) => x`) is read again as code, each `<` only once; a file that would need more of that read twice than it is long is read to its end without elements.
