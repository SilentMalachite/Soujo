...
- `soujo map code` reads a JSX element as an element outside `.ts`, `.mts`, and `.cts`, where `<x>` is a type instead: its text and its quoted attribute values hold no code, so `<p>from './a.js'</p>` is no longer drawn as an edge and an apostrophe in the text (`<p>don't</p>`) no longer starts a string that swallows what follows. What a `{ }` inside an element holds is still read as code. Value and type positions are not told apart, so a generic function type where a value can start (`type X = <T>(a: T) => T`) is read as an element; nothing closes it, and every import after it outside a `{ }` is lost. What cannot be in a tag (`<T,>(x) => x`) is read again as code.
