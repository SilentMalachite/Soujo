...
Release: [v0.7.0](https://github.com/SilentMalachite/Soujo/releases/tag/v0.7.0).
...
- CI runs the suite on Windows as well as Ubuntu and macOS, where the tests for path comparison, symlinks, `.git` under its other names, the plugin directory, and `headState` skip themselves and so were never run on the platform they are written for. Six things stood in the way, all in the tests rather than in the CLI: `.gitattributes` now keeps LF in the working tree, since a CRLF checkout made the committed `dist/` differ from what tsc writes and broke every reader of SKILL.md frontmatter; the tests point `GIT_CONFIG_GLOBAL` at an empty file rather than the null device, which git cannot open as `\\.\nul` and which failed every git call; `gitToplevel` is compared through a path spelled one way, since `realpathSync` keeps the 8.3 short name and the backslashes where git prints the long path with `/`; the hook test substitutes `${CLAUDE_PLUGIN_ROOT}` itself, as the host does, because `cmd.exe` leaves it as written; and four tests that need a name or a permission Windows has no equivalent for — a directory name ending in a space, a `?` in a file name, `chmod 0`, and an argument of 100 000 characters — skip themselves there.
...
- The `spec` skill settles the principles in one question, from candidates drawn from the answers, the config files, and the conventions the repository states, and writes `SPEC.md` in five sections with principles as `- P<n> <名前> — <判定できる1文>` (at most 7) and top-level acceptance criteria as `- A<n> <判定できる1文>`. Keys are never reused or renumbered; an existing SPEC has its principles and criteria put in these forms without their meaning changing, a principle getting a name as well as a key, and a missing heading is added. When the principles are not settled, their section is left empty rather than `未定`, which `soujo next check` would warn about. The `節目` entry says which principles changed, and its open line names principles not settled.
- The `go` skill goes on without confirmations except where the completion condition cannot hold without breaking a principle of `SPEC.md`; there it asks one question naming the principle, with numbered candidates.
- The `review` skill and the `soujo:reviewer` agent read the principles of `SPEC.md` and list their violations first, one per row, each starting with its key (`P<n>`, or the section and number in a SPEC without keys), since review shows the reviewer's table unedited.
- CLAUDE.md / AGENTS.md and their templates say that the principles of `SPEC.md` outrank its other sections, the completion conditions, and existing code, and when to ask; the principles themselves are written only in SPEC. The READMEs name the principles in the skill table and the `SPEC.md` row.
