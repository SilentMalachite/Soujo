...
- committing files the user did not mean to commit (`layer done`, `close`, and `log rotate` stage every change in the project, and refuse only untracked files named like credential files that git does not ignore; `phase done` stages only the records, with their symlink targets and, for `spec`, the `CLAUDE.md` / `AGENTS.md` of `soujo init` while unchanged and not in HEAD, and refuses an untracked target named like a credential file);
