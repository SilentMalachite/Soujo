...
import { ARCHIVE_PATHSPEC, archiveFile, archiveFiles, archiveMonth, readState, removeLeftoverTemps, requireStateDir, statePath, writeState, } from '../files.js';
...
function stoppedRotation(root, dir, log) {
    if (gitStatusExcluding(root, []).length === 0)
        return [];
    if (gitStatusExcluding(root, [statePath('LOG.md'), ARCHIVE_PATHSPEC]).length > 0)
    ...
    if (uncommittedLogs(log, head).length > 0 || lost.length > 0)
        throw new Error(DIRTY);
