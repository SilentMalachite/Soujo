...
import { formatNext, parseNext, parsePlan, validateNext } from '../state.js';
...
export function nextSet(cwd, input) {
    ...
    if (problems.length > 0)
        throw new Error(`NEXT.md を書かない: ${problems.join('、')}`);
...
function problems(dir) {
    ...
    if (next === undefined) {
        ...
        if (layer !== undefined && plan !== undefined && parsePlan(plan).some((item) => item.done && item.layer === layer)) {
...
export function nextCheck(cwd, hook) {
    let found;
    try {
