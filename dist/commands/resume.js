...
import { PHASES, daysBetween, lastLog, newlyDone, nextLayer, nextStatus, parseNext, parsePlan, specUnwritten, validateNext, validatePlan, } from '../state.js';
import { attempt, clip, commandArg, describeInvalidNext, headState, optionArg, readHead, skill, statePaths, uncommittedPaths, } from './shared.js';
...
function pendingPhase(root, dir) {
    ...
    if (phase === undefined || !PHASES.includes(phase) || next.layer === phase)
        return undefined;
...
function unfinishedPhase(root, dir) {
    const phase = attempt(() => pendingPhase(root, dir));
    if (phase === undefined || phase instanceof Error)
        return undefined;
...
export function resume(cwd, now = new Date()) {
