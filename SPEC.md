...
| `soujo init` | Creates `.soujo/` from templates, plus CLAUDE.md / AGENTS.md if missing. Uses the git top level when inside a repository. Never overwrites existing files. Refuses and creates nothing when a state file's real path (through a symlink of the file or of `.soujo/`) is outside the project, inside `.git`, or another state file's | Created files, then one line listing skipped ones, and a line saying `git init` is needed outside a repository |
