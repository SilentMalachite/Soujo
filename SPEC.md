...
| `LOG.md` | Journal: `## YYYY-MM-DD <layer>` (a calendar date) followed by up to 3 lines. Commands only append, except `soujo log rotate`, which moves past months into archives | 3 lines per entry | `soujo log add` / `layer done` / `close --note`; `soujo log rotate` removes |
