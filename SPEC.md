...
| `PLAN.md` | List of layers: `- [ ] <layer> — <completion condition>`. Checklist items inside a code fence are examples, not layers | 1 line per layer | `plan` skill; checked by `soujo layer done` |
...
- Output is short and in Japanese; normally at most 5 lines on stdout (`--help` and `map` print more). Errors are one line on stderr with exit code 1. Output and errors alike show a path under the home directory as `~` (in either normalization, with either separator, and in either letter case only where the file system ignores case), so that a copied line carries no user name. A stream closed before the write (`soujo resume` piped into `head -1`) ends the run quietly instead of with an `EPIPE` stack trace; any other failed write is reported on stderr in one line with exit code 1. A layer name inside a command a message suggests is a single-quoted argument, as the skills spell it, with `--` before one starting with `-`.
- Every command needs the current directory, since `.soujo/` is searched from there. When it cannot be read — removed while a shell stayed in it, or no longer permitted — `next check` (with or without `--hook`) and `next show --hook` print nothing and exit 0, as they do outside a project, and every other command says so in one line and exits 1, naming the reason's code when it is not `ENOENT`. `--help` is unaffected.
- Behaves the same on any host. No host detection; only `--hook` switches to a Claude Code hook-friendly output.
- `.soujo/` is searched upward from the current directory, stopping at the git top level (a directory containing `.git`). Outside a repository only the current directory counts, as `soujo init` creates `.soujo/` there.
...
| `soujo next show [--hook]` | Prints `NEXT.md`, or `NEXT.md なし` when the file is missing, followed by one line naming the problems of `NEXT.md` itself (`validateNext`, built as `next check` builds its line: the first four, then `ほか<N>件`), since `resume` and `close` refuse such a file and its text does not say why. The lines printed are the ones the 5-line limit counts, so a blank line at the end is shown as the line the warning names, and a file that is there but empty gives the warning alone. With `--hook`, prints nothing when `NEXT.md` is missing | the lines of `NEXT.md`, plus one when invalid |
| `soujo next set --layer --premise --check [--caution] [--effort]` | Rewrites `NEXT.md`. Defaults: caution `なし`, effort `medium`. For the layers `spec` / `plan` (compared exactly after trimming) the effort is `high` (§7): an `--effort` other than `high` is refused, nothing is written, and the error says to omit `--effort`. Values must be one line without control characters (§14). When PLAN has layers, a layer not in PLAN (other than `spec` / `plan`) is refused and nothing is written. While PLAN leaves a code fence unclosed, has an empty layer name, a layer name with control characters (all three named by their line), repeats a layer name, or has a layer named `spec` / `plan` / `節目`, any layer is refused and nothing is written | 1 line |
| `soujo next check [--hook]` | Warns when `NEXT.md` is missing, invalid, points to a layer already `[x]` in PLAN or past an unchecked one (`plan` comes after every layer), PLAN leaves a code fence unclosed, has an empty layer name, a layer name with control characters (all three named by their line), repeats a layer name, or has a layer named `spec` / `plan` / `節目`, a layer has no completion condition (named by its line; a warning here, a refusal in `layer done`), the `確認:` of `NEXT.md` is not the completion condition `PLAN.md` gives its layer while that layer stands (whitespace runs compare equal, control characters among them; nothing is compared for a phase, a layer PLAN has not got or repeats, one with no condition, or a layer already done or reached too early, whose own warning is the one to act on), there are uncommitted changes in the project (an untracked directory counts once, whatever `status.showUntrackedFiles` says; past 16 MiB of `git status` output the count is shown as `N件以上`), `soujo log rotate` would move entries of two or more months, or one of `NEXT.md`, `PLAN.md`, and `LOG.md` cannot be read (each reported with the other warnings, only the checks needing that file left out). The line names the first four problems and then `ほか<N>件`, since problems named by line are one per broken line. Silent outside Soujo projects. `--hook` returns `{"systemMessage": "..."}`, built after the home directory is hidden. **Exit code is always 0** | 0–1 line |
...
| `soujo log rotate [--before YYYY-MM]` | Moves the entries of `LOG.md` dated in a month before the current one (or `--before`; `YYYY-MM`, month 01–12) into `.soujo/LOG-YYYY-MM.md`, one archive per month by the entry's date, appended to a new or existing archive with their lines as `LOG.md` has them. Kept in `LOG.md` whatever the date: the text before the first entry, the last entry (so `resume` still shows `前回:`), the last `節目` entry (so `brief` still shows `節目:`), and entries whose date names no month. An entry the archive of its month already has is not appended again, compared by content (date, layer, and lines), which is all there is to tell two entries apart: entries `LOG.md` repeats word for word each move, but a copy the archive already holds is removed from `LOG.md` without being appended, so that many copies end as one. Archives are written first and `LOG.md` last, then the project is committed as `log: rotate <month>` or `log: rotate <first>..<last>`, the months of the entries `LOG.md` loses in the commit. Refuses and writes (or deletes) nothing when: `--before` is not a month; the same uncommittable states as `layer done`; an archive's real path is outside the project, inside `.git`, or another state file's or archive's, or an archive to write or commit is git-ignored; there are uncommitted changes other than a stopped rotate's: changes outside `LOG.md` and existing archives (leftover temporary files aside), a `LOG.md` that is not HEAD's with whole entries removed, a changed archive that is not HEAD's with at least one whole entry of its month appended, an archive new since HEAD whose text before its entries is not the `# LOG <month>` heading a rotate writes, a removed or appended entry that no rotate of HEAD's `LOG.md` moves (the last entry or the last `節目` entry, a date naming no month, an entry `LOG.md` never had), or a removed entry not in the archive of its month; or a stopped rotate's entries are not all moved with this `--before` (`前回と同じ --before で再実行する`). So re-running after a failed write or commit finishes the same rotation, also when the archives were committed by hand in between. With nothing to move or commit it only removes leftover temporary files and prints `移動なし` | 2 lines: `LOG.md から N件を M書庫へ移動（<months>）` and the commit |
| `soujo layer done <layer> [--note ...]` | Checks PLAN → appends LOG → `git add -A -- .` and `git commit -m "layer: <layer>" -- .` in the project directory (a project in a subdirectory commits only that subdirectory). Refuses and writes nothing when: the layer is not in PLAN, PLAN leaves a code fence unclosed, has an empty layer name, a layer name with control characters (all three named by their line), repeats a layer name, or has a layer named `spec` / `plan` / `節目`, or the note breaks LOG limits or starts with `中断:` (reserved for `close`); its PLAN item has no completion condition, which is said after the checks for an already committed layer, so that one closed before its condition was removed is still reported as committed; `NEXT.md` is missing, invalid, or `次:` is still this layer; not a repository, `.soujo/` is a symlink, a state file's real path (a symlink's target) is outside the project, inside `.git`, or another state file's or archive's, a merge/rebase/cherry-pick/revert is unfinished (a leftover `sequencer/` included), files in the project are unmerged (a conflict elsewhere in the repository does not stop git from committing the project, nor this), `.soujo/` files or their symlink targets are git-ignored, or an untracked file git does not ignore has a credential file name (`.env` and `.env.*` other than `.env.example`, `.npmrc`, `.netrc`, `_netrc`, `.git-credentials`, `.credentials.json`, `auth.json`, the `id_rsa` / `id_ed25519` / `id_ecdsa` keys, `*.pem`, `*.key`, in any letter case where the file system ignores case; one added with `git add` first is committed); the layer is already committed (a `layer: <layer>` commit changing the project exists, or it is checked in PLAN at HEAD). When the check exists only in the working tree (a previous run stopped), appends LOG unless LOG has this layer's completion entry not in HEAD yet, wherever it is (a `中断` entry of `close` does not count), and commits. Nothing is committed while SPEC, PLAN, LOG, or NEXT is not staged as written (e.g. skip-worktree). A failure after writing, a git hook refusing the commit included, says what is recorded; fixing the cause and re-running resumes | 1 line, with up to 5 added files |
| `soujo resume` | `次: <layer>（effort: <e>）確認: <check>` (`NEXT.md`) / `前回: <date> <layer> — <first line>` (last `LOG.md` entry) / `コミット: <hash> <subject>（未コミット N件）` / `再開: /soujo:go（Codex は $go）`. `コミット:` is the last commit changing the project (`まだない` before one), with `N件以上` past the count limit of `next check`. Values, layer names included, are cut at 60 characters with `…`; a layer name inside a command stays whole and single-quoted. While PLAN has a problem `next set` refuses, before any other hint: `次: 不明（PLAN.md が無効: …）` and a `再開:` saying to fix it and run `soujo resume` again. When `NEXT.md` is missing, unreadable, invalid, or points to a checked layer: `次:` is PLAN's next layer and `再開:` gives the reason and `soujo next set`; with no layer left, `/soujo:spec` (SPEC.md missing or still the template) or `/soujo:plan`. When `NEXT.md` points past an unchecked layer (`次: plan` included): `次:` is that layer and `再開:` suggests `soujo layer done`. A PLAN check not committed yet (a stopped `layer done`) makes `再開:` say to re-run it. Unreadable files and git failures degrade only their line. When the last commit is 3 or more calendar days old (local dates, as `brief` prints them), `再開:` ends with `・N日ぶり: 先に soujo brief` | 4 lines |
| `soujo brief` | Where the project stands, for a return after days or weeks away, derived from the records (nothing is written): `進捗: PLAN <done>/<all> 層完了・最終 layer: <date> <layer>` (the latest `layer:` commit changing the project; `なし` before the first) / `以後: layer 後のコミット N件: <subjects>` (up to 3 subjects, oldest first, clipped as in `resume`, then `…` when there are more; `なし` when the last commit is the layer commit; every commit as `最初からのコミット N件` before the first layer commit) / `節目: <date> — <first line>` (the last `節目` entry of `LOG.md`, or `なし`) / `空白: 最終コミットから N日（<date>）` (calendar days in local time) / `次: <layer>（effort: <e>）確認: <check>` as `resume` prints it. Unreadable files and git failures degrade only their line, as in `resume` | 5 lines |
| `soujo close [--note ...]` | Logs `--note` as `中断: ...` → commits the project as `wip: <layer>` → prints how to resume. Refuses (exit 1) and writes nothing when: the same uncommittable states as `layer done`; the PLAN problems `layer done` refuses (a code fence left open, an empty or repeated layer name, one with control characters, a layer named `spec` / `plan` / `節目`); `NEXT.md` is missing, invalid, or points to a checked layer; a PLAN check is not committed yet (re-run `layer done` instead); the note is blank or breaks LOG limits. Nothing is committed while SPEC, PLAN, LOG, or NEXT is not staged as written (e.g. skip-worktree). The layer is `次:`, or PLAN's first unchecked layer when `次:` already points past it (stopped between `next set` and `layer done`). A `中断` entry of that layer not in HEAD yet, wherever it is in LOG, is kept instead of adding another, so re-running after a failed commit retries only the commit | 2 lines |
...
| `soujo map code [dir]` | Mermaid `graph LR` of the relative imports in the main language (most files) under `dir` (default: the project root, or cwd outside Soujo projects). Languages are one row each in `LANGUAGES` in `src/map.ts`; only rows with import rules (TS/JS initially) are drawn, and other main languages or no recognized file give an ASCII directory tree. Package and path-alias imports, and specifiers that are not one fixed string (`'./a' + b`), are not drawn; escapes in a specifier are decoded, and a specifier is cut at its first `?` or `#`, whichever comes first, before the percent escapes of what is left are decoded; the text as written is tried after the decoded one, so both `d.e.js` and a file really named `d%2Ee.js` are found. Paths are compared as the file system holding the scanned files compares them, asked once per run of a file that was scanned: always in NFC, since a file system may hand back either normal form for one name, and in one letter case only where it folds case (where that cannot be measured, the platform decides). The graph therefore follows what that file system resolves — a specifier of another letter case is an edge where the file system finds it and none where it does not — rather than being the same everywhere. An extension is swapped for another only where it is written as TypeScript writes it (`./a.js`, not `./a.JS`), so no swap names a spelling the specifier does not have. JSX is read in `.tsx`, `.jsx`, `.js`, `.mjs`, and `.cjs`, where an element's text and quoted attribute values hold no import and only what its `{ }` hold is code; in other extensions `<x>` is a type. Value and type positions are not told apart, so a generic function type where a value can start (`type X = <T>(a: T) => T`) opens an element; as nothing closes it, it is read again as code from its `<` once the file ends (what is read twice is capped at the file's length, past which the rest is read without elements). Skips `node_modules`, `dist`, `build`, `target`, `vendor`, `deps`, `_build`, `__pycache__`, `venv`, `coverage`, dot-entries, and symlinks; unreadable subdirectories and files are skipped and counted. A `dir` whose real path is outside the project's (cwd's where there is no project), compared as the file system compares names, is read as given, and the first note says so. In a label, `"` `#` `&` `<` `>` `]` and the backtick are written as entity codes, so that no file name ends its node. Capped at 5000 scanned entries, 524288 bytes (512 KiB) scanned from one file (one more byte is read, to tell a file at the limit from one beyond it; a cut file is scanned only to its last complete line, since the cut lands where the byte count falls, inside a character or a token), 100 files (most connected kept), 300 edges, and 200 tree lines, with a note | Mermaid or tree |

Public pure functions of `state.ts` (each has tests):
`parseNext` / `formatNext` / `validateNext` / `parsePlan` / `planLayers` / `validatePlan` / `missingConditions` / `checkMismatch` / `formatItem` / `nextLayer` / `nextStatus` / `newlyDone` / `markDone` / `printable` / `logLines` / `appendLog` / `parseLog` / `lastLog` / `isMonth` / `requireMonth` / `logMonth` / `logMonths` / `rotateLog` / `archiveLog` / `appendedEntries` / `removedEntries` / `lastMilestone` / `daysBetween` / `formatDate`.
...
- Staging, commits, the uncommitted count, and the check for unmerged files are limited to the project directory (`-- .`), since git commits the project while a file elsewhere is unmerged; an unfinished merge, rebase, cherry-pick, or revert refuses wherever it is. Output lines never carry control characters.
...
- `layer done` and `close` check that SPEC, PLAN, LOG, and NEXT are staged as written before committing, because after a commit without them a re-run of `layer done` is refused as already committed, `close` would leave a `wip:` commit without its `中断` entry or with an old `NEXT.md`, and a `SPEC.md` the skills changed would be left out of its layer without a word.
...
- A layer name is not empty and carries no control characters, and a layer has a completion condition. The first two are reported by their line, since neither can be quoted back in a message or matched when pasted into a command, and the line is what there is to fix; `layer done` refuses the third by its line too, because a layer with nothing to close it against leaves the `確認:` line of the next `NEXT.md` blank, while `next check` only warns about it, so that a `PLAN.md` written before the rule still resumes and the layer is named before the half hour of work it takes to reach `layer done`. Checklist items inside a code fence are not layers at all, since a format example written into `PLAN.md` by hand or by another tool would otherwise be counted or checked as one and shift every layer after it; a fence left open swallows the rest of the file, so it is reported by its line rather than letting those layers go missing without a word. A fence is the only container of examples `PLAN.md` has: an item indented by four spaces, or one inside an HTML comment, is still a layer. A layer whose separator is left with nothing after it (`- [ ] L2 state —`, the usual way the condition is forgotten) keeps the separator out of its name and has no condition.
- The `確認:` of `NEXT.md` and the completion condition of its layer in `PLAN.md` say the same thing in two files, because `NEXT.md` is what a session resumes from on its own and `PLAN.md` is where the condition is decided: `next set` copies one into the other and they part when either is edited afterwards. `next check` warns when they differ, and the `plan` skill runs it after rewriting `PLAN.md`, so that a layer is not closed against a condition the plan no longer holds. Whitespace runs compare equal, so re-spacing or rewrapping a condition is not a difference, and nothing is compared where the two are not meant to match: a phase, a layer `PLAN.md` has not got or repeats, one left without a condition, and a layer already done or reached too early, whose own warning is the one to act on. A `--check` reworded rather than copied warns for as long as the layer lasts, which is the cost of not refusing it: a condition is prose, and `next set` cannot tell a better wording from a different promise.
- A control character is a C0 control (U+0000–U+001F, tab, CR, and LF included), DEL (U+007F), a C1 control (U+0080–U+009F, which a terminal decoding Latin-1 takes for an escape sequence), or a line separator (U+2028/2029). Output lines, `LOG.md` lines, `NEXT.md` values, and the layer names `log add` and `layer done` accept all use that one range. A `NEXT.md` value carrying one is refused rather than flattened, since `次:` becomes the subject of a `wip:` commit.
...
- Commits run the repository's git hooks (no `--no-verify`), because a hook guarding what is committed, such as a secret scanner, must guard layer and `wip:` commits too. A hook that refuses fails the commit after the records are written, which the error says, and re-running after fixing the cause commits them.
- `layer done`, `close`, and `log rotate` stage the whole project, so before writing anything they refuse an untracked file with a credential file name that git does not ignore: the project's `.gitignore` is the user's, and an entry forgotten there would put a key into history. The names are those this repository's privacy test checks; tracked files and files added with `git add` are the user's choice and are committed.
- Outside a repository `.soujo/` is looked for in the current directory only (§6), where `soujo init` creates it, because walking up would make a `.soujo/` made in the home directory the project of every directory below it.
- Days away (`N日ぶり` of `resume`, `空白` of `brief`) are calendar days in local time, the dates `formatDate` prints, so that a count agrees with the date shown beside it.
- The uncommitted count reads at most 16 MiB of `git status` output and then says `N件以上`, so that a huge working tree does not fill the memory of the Stop hook; other git output keeps the 256 MiB a long history needs.
- `soujo map code` reads a `dir` outside the project as given and says so in its first note; the `map` skill passes one only when the user names it.
