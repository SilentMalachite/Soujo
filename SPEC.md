...
| `soujo next check [--hook]` | Warns when `NEXT.md` is missing, invalid, points to a layer already `[x]` in PLAN or past an unchecked one (`plan` comes after every layer), PLAN repeats a layer name or has a layer named `spec` / `plan`, or there are uncommitted changes in the project (an untracked directory counts once, whatever `status.showUntrackedFiles` says). Silent outside Soujo projects. `--hook` returns `{"systemMessage": "..."}`. **Exit code is always 0** | 0–1 line |
