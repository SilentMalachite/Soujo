...
- A control character is a C0 control (U+0000–U+001F, tab, CR, and LF included), DEL (U+007F), a C1 control (U+0080–U+009F, which a terminal decoding Latin-1 takes for an escape sequence), or a line separator (U+2028/2029). Output lines, `LOG.md` lines, and the layer names `log add` and `layer done` accept all use that one range.
