...
| `resume` | — | Return the CLI output as is; when `再開:` says `N日ぶり`, run `soujo brief` too | `soujo resume` → `soujo brief` (only when `再開:` says `N日ぶり`) | 4 lines, then the 5 lines of `brief` when it ran | low |
