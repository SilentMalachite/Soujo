...
| `LOG-YYYY-MM.md` | Archive of `LOG.md`: the entries of one month, same format, under `# LOG YYYY-MM`. Only `soujo log rotate` reads them | 3 lines per entry | `soujo log rotate` |
...
| `soujo next check [--hook]` | Warns when `NEXT.md` is missing, invalid, points to a layer already `[x]` in PLAN or past an unchecked one (`plan` comes after every layer), PLAN repeats a layer name or has a layer named `spec` / `plan`, there are uncommitted changes in the project (an untracked directory counts once, whatever `status.showUntrackedFiles` says), or `LOG.md` holds entries of three or more months (`soujo log rotate`). Silent outside Soujo projects. `--hook` returns `{"systemMessage": "..."}`. **Exit code is always 0** | 0–1 line |
...
| `soujo log rotate [--before YYYY-MM]` | Moves the entries of `LOG.md` dated before the current month (or `--before`) into `.soujo/LOG-YYYY-MM.md`, one archive per month by the entry's date, appended to an existing archive as `LOG.md` has them. Kept in `LOG.md` whatever the date: the text before the first entry and the last entry, so `resume` still shows `前回:`. Archives are written first and `LOG.md` last, then the project is committed as `log: rotate <months>` (the months of the archives the commit changes). Refuses and writes nothing when: `--before` is not `YYYY-MM`; the same uncommittable states as `layer done`; an archive to write has its real path outside the project or inside `.git`, or is git-ignored; there are uncommitted changes other than a stopped rotate's (only `LOG.md` and archives changed, `LOG.md` has no entry HEAD lacks, and every entry it lost is in an archive's uncommitted entries). An entry already among an archive's uncommitted entries is not appended again, so re-running after a failed write or commit finishes the same rotation. Nothing to move or commit prints `移動なし` | 1 line per archive written, then the commit |
...
- `soujo log rotate` moves whole months of `LOG.md` into `LOG-YYYY-MM.md` by entry date, not position, and always keeps the last entry (L15), because `resume` reads `前回:` from `LOG.md` and no status command reads the archives. It is manual, not part of `layer done`, and refuses other uncommitted changes because `commitRecords` stages the whole project. A stopped rotate's changes are the one exception, because the Codex sandbox cannot commit without approval and a re-run must finish rather than be refused. `uncommittedLogs` compares entries by content, so a rotated `LOG.md` does not look like uncommitted entries to `layer done` / `close`. Archive names are validated as `LOG-\d{4}-\d{2}\.md` and read and written through the same in-project checks as the four state files.
