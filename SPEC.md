...
- A layer name is not empty and carries no control characters, and a layer has a completion condition. The first two are reported by their line, since neither can be quoted back in a message or matched when pasted into a command, and the line is what there is to fix; `layer done` refuses the third, because a layer with nothing to close it against leaves the `確認:` line of the next `NEXT.md` blank. Checklist items inside a code fence are not layers at all, since an example of the format written into `PLAN.md` would otherwise be counted or checked as one and shift every layer after it; a fence left open swallows the rest of the file, so it is reported by its line rather than letting those layers go missing without a word. A layer whose separator is left with nothing after it (`- [ ] L2 state —`, the usual way the condition is forgotten) keeps the separator out of its name and has no condition.
