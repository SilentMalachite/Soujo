...
function logSeparator(text: string): string {
  if (text === '') return '';
  if (!text.endsWith('\n')) return '\n\n';
...
export function appendLog(text: string, entry: LogEntry): string {
  ...
  if (layer === '' || /[\r\n]/.test(layer)) throw new Error('層名は空でない1行で書く');
...
export function lastLog(text: string): LogEntry | undefined {
  ...
  for (const raw of text.split('\n')) {
    ...
    if (header) last = { date: header[1] ?? '', layer: (header[2] ?? '').trim(), lines: [] };
    else if (last && line !== '') last.lines.push(line);
  }
  return last;
}
