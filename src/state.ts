...
export function parsePlan(text: string): PlanItem[] {
  ...
  for (const line of text.split('\n')) {
    ...
    if (match) items.push({ ...splitItem(match[2] ?? ''), done: match[1] !== ' ' });
  }
  return items;
}
...
export function nextStatus(layer: string, items: PlanItem[]): NextStatus {
  ...
  if (index === -1) return { state: 'ok' };
  if (items[index]?.done) return { state: 'done' };
...
export function markDone(text: string, layer: string): string {
  ...
  if (line === undefined) throw new Error(`PLAN.md に層「${layer}」がない`);
...
export function parseLog(text: string): LogEntry[] {
  ...
  for (const raw of text.split('\n')) {
    ...
    if (header) entries.push({ date: header[1] ?? '', layer: (header[2] ?? '').trim(), lines: [] });
    else if (line !== '') entries[entries.length - 1]?.lines.push(line);
  }
  return entries;
}
