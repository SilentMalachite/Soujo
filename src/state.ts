 ...
 */
export const AFTER_LAYERS: readonly string[] = ['plan', 'converge'];
 ...
 */
function readComments(line: string, open: boolean): boolean | undefined {
  let position = 0;
  let inside = open;
  for (;;) {
    if (inside) {
    ...
    while (position < line.length && /\s/.test(line.charAt(position))) position += 1;
    if (position === line.length) return false;
    if (!line.startsWith('<!--', position)) return undefined;
    position += '<!--'.length;
    inside = true;
  }
}
 ...
 */
export function specUnwritten(text: string): boolean {
  let open = false;
 ...
 */
export function missingMilestone(layer: string, items: readonly PlanItem[], log: string): string | undefined {
  if (!AFTER_LAYERS.includes(layer.trim())) return undefined;
  const layers = new Set(items.map((item) => item.layer));
  for (const entry of parseLog(log).reverse()) {
    if (entry.layer === MILESTONE) return undefined;
    if (layers.has(entry.layer)) return `LOG.md に層「${entry.layer}」の記録より後の節目がない`;
  }
