...
export function requireStateDir(start: string): string {
...
interface Step {
  /** Relative to root, "/"-separated; it starts with ".." when it is above root. */
  path: string;
  link: boolean;
}
 ...
 */
function stepsOf(root: string, path: string): Step[] {
  const visited: Step[] = [];
  ...
  for (let links = 0; pending.length > 0; ) {
    ...
    if (link) {
      ...
      if (text === undefined) return visited;
 ...
 */
export function stateLinks(root: string, file: StateFile): string[] {
...
function tempPid(target: string, name: string): number | undefined {
...
function tempsOf(target: string, running = false): string[] {
  let entries;
  try {
 ...
 */
export function isPlacedRootFile(root: string, file: RootFile): boolean {
...
function leftoverTempPaths(dir: string, running = false): string[] {
...
function relativeTemps(dir: string, running: boolean): string[] {
 ...
 */
export function stateTemps(dir: string): string[] {
