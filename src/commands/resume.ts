...
import { attempt, clip, commandArg, describeInvalidNext, headState, readHead, skill, type NoCommit } from './shared.js';
...
export function nextAndCommand(dir: string, plan: string | undefined | Error): [string, string] {
  ...
  if (items.length > 0) return ['次: なし（PLAN は全層完了）', `再開: ${reason} → 層を足すなら ${skill('plan')}`];
