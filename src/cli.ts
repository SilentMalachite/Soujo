...
function currentDir(): string | NodeJS.ErrnoException {
  try {
    return process.cwd();
  } catch (error) {
...
function noCurrentDir(error: NodeJS.ErrnoException): string {
...
function main(argv: string[]): number {
  try {
    ...
    if (lines.length > 0) write(process.stdout, shown(lines));
    return 0;
  } catch (error) {
