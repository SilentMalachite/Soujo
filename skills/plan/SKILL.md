...
- 層名は英数字の短い名前（例：`L3 auth`）。` — `・` - ` のような区切りや引用符を含めず、空にせず、制御文字（タブ・行区切り）を含めず、PLAN の中とも過去の `layer:` コミットとも重複させず、`spec` / `plan` にも、LOG の節目と紛れる `節目` にもしない（CLI が拒否する）。書式の例を残すならコードフェンスの中に書く（4スペースの字下げや HTML コメントの中は層として数える）。
